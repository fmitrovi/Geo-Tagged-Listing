...
library(ggsn)
 ...
 df <- na.omit(object = df)
    ...
    tuvmapea <- get_map(location = c(lon = mean(df$lon), lat = mean(df$lat)), zoom = 17,
  ...
  # Plotting the Location of the EA
   ...
   tuvmapea_small <- get_map(location = c(lon = mean(df$lon), lat = mean(df$lat)), zoom = 15,
                       source = "google" , maptype = "hybrid", scale = 1)
   ...
   ggsave(filename = "Cluster_map_maintuvalu_711.pdf", plot = last_plot(), device = NULL, path = NULL,
